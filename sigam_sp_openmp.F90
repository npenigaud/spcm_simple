...
!$OMP DO SCHEDULE(STATIC)
  ...
  DO JLEV=1,KLEV
    ...
    ZDETAH=-YDVETA%VFE_RDETAH(JLEV)*YDCST%RD*SILNPR(JLEV)
    !$acc loop vector
    DO JSPEC=1,KSPEC
      !v1 ZSPHI(JSPEC,JLEV)=-YDCST%RD*PT(JLEV,JSPEC)*SILNPR(JLEV)*ZDETAH
      !v2 ZSPHI(JSPEC,JLEV)=PT(JLEV,JSPEC)*SILNPR(JLEV)*ZDETAH
...
!$OMP DO SCHEDULE(STATIC)
  ...
  !DO JLEV=1,KLEV
  DO JSPEC=1,KSPEC
     intermediaire=PSP(JSPEC)*SIRPRG
  !$acc loop vector
    !DO JSPEC=1,KSPEC
     DO JLEV=1,KLEV
